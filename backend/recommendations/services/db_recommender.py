...
def score_place(place, tag_data, categories, preferred_tags, distance):
    ...
    score = 20 + category_score + tag_score + distance_score + quality_score - warning_penalty
...
def build_db_recommend_reason(place, scenario, distance, matched_tags, match_level):
    parts = []
...
def search_db_recommendations(
    ...
    if lat is not None and lng is not None:
        nearby = []
        for place in base_places.only("id", "lat", "lng").iterator(chunk_size=2000):
