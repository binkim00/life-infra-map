...
- 주차장: 무료 명시 `무료이용`, 장애인 전용구역 보유 명시 `장애인전용주차`, 모든 운영일의
  00:00~23:59/24:00 명시 `24시간운영`, 결제방법에 카드가 명시되면 `카드결제가능`
- 도시공원: 해당 시설 문자열이 실제 존재할 때만 `놀이시설`, `운동시설`, `편의시설`

공식 Evidence row는 144건에서 53,714건으로 증가했다. 이 중 53,409건은 positive이고,
`연중무휴`를 24시간으로 과잉 해석했던 305건은 삭제하지 않고 neutral/needs_verification으로
무효화했다. `PlaceTagEvidence` 전체는 400건에서 53,970건, Evidence 보유 Place는 207곳에서
32,767곳으로 증가했다. 원본에 없는 값이나
...
## 2026-08-16 공식 Source 확대와 one-off 2,000곳 Bootstrap

전국도서관표준데이터 3,526건 가운데 좌표가 있는 3,378건을 단계적으로 검증한 뒤 Place로
연결했다. 기존 Kakao 도서관 10곳과 공식 Place 3,368곳이며, 좌표가 없는 148건은 만들지 않았다.
`Place.raw`에는 SourcePlaceRecord의 실제 원본 필드를 보존한다. 좌석 기준은 임의 상수가 아니라
유효 좌석 수 분포의 75분위인 150석으로 문서화하고 설정 가능하게 유지했다. 생성한 공식
Evidence는 5,080건이다.
...
TourAPI 상세는 실제 승인 상태를 다시 확인한 뒤 10, 100, 400건 순으로 호출했다. 총 510회 중
상세가 보존된 Place는 504곳이며 실패와 429는 없었다. 응답에 직접 있는 주차·카드결제 필드만
사용했고 관광지 성격으로 분위기나 전망을 추론하지 않았다. 공중화장실 최신 전국표준데이터는
공식 페이지에서 52,255건과 일일 갱신 상태를 확인했지만 이 환경의 직접 파일 다운로드가 403이라
기존 Place를 갱신하지 않았다. 오래된 Evidence도 삭제하지 않았다.

공식 무료와이파이는 83,145곳으로 아직 Evidence가 없던 가장 큰 구조화 Source였다. 실제 raw의
`와이파이SSID`가 있는 62,131곳에만 `무료와이파이` Evidence를 만들었다. 기준일 기반 400일 TTL을
적용한 결과 26,777건은 current, 35,354건은 stale이다. Category가 freewifi라는 이유만으로
Evidence를 만들지 않는다. beach 공식 8곳은 이름·좌표 외 직접 Feature가 없어 보강하지 않았다.

주관 태그 one-off 실행은 운영 기본값을 바꾸지 않고 `500 → 2,000`으로 확대했다. 2,000곳은
Naver 4,499회, 857.48초, 실패/429/retry 0으로 끝났고 413곳(20.65%)에서 웹 Evidence 938건과
웹 PlaceTag 227건(candidate 38, needs_verification 189)을 새로 만들었다. Category별 Hit Rate는
