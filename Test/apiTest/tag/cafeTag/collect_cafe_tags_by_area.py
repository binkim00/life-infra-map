...
# 실행 위치가 달라도 현재 파일 옆의 .env를 우선 읽도록 처리
...
# 태그가 하나도 없으면 최종 저장하지 않음
REQUIRE_TAGS_FOR_SAVE = True

# API 과호출 방지를 위한 대기 시간
KAKAO_REQUEST_DELAY = 0.2
NAVER_REQUEST_DELAY = 0.2
PLACE_REQUEST_DELAY = 0.3
...
def make_skip_place(area, place, reason, query=None):
    return {
...
def collect_kakao_places_for_area(area):
    all_places = []
    seen_ids = set()
    skipped_places = []
...
def build_blog_queries(place_name, address, area_name):
    address_parts = address.split()
...
def is_related_blog_item(place_name, address, area_name, item):
    title = clean_text(item.get("title", ""))
    description = clean_text(item.get("description", ""))

    text = normalize_for_match(title + " " + description)
    normalized_place_name = normalize_for_match(place_name)
    normalized_area_name = normalize_for_match(area_name)
    ...
    has_area_name = normalized_area_name in text if normalized_area_name else False

    # 장소명이 확실하고 지역명 또는 주소 키워드가 같이 있으면 관련 글로 판단
    return has_address_keyword or has_area_name
...
def analyze_blog_items(items):
    ...
    for item in items:
        ...
        # 태그 판단은 요약을 우선 사용하되, 제목도 보조로 함께 확인
        tag_text = f"{title} {description}".strip()
        link = item.get("link") or tag_text
    ...
    for tag_name, links in tag_hits.items():
        ...
        tags.append({
            ...
            "source": "naver_blog_title_description",
            "is_ai_generated": False,
            "is_verified": False,
        })

    tags.sort(key=lambda tag: (-tag["confidence"], tag["name"]))

    return tags


def collect_tags_for_cafe(area, place):
    place_name = place.get("place_name", "")
    address = place.get("road_address_name") or place.get("address_name") or ""

    all_blog_items = []

    for query in build_blog_queries(place_name, address, area["area_name"]):
        try:
            ...
            for item in items:
                if is_related_blog_item(place_name, address, area["area_name"], item):
    ...
    return {
        ...
        "category": place.get("category_name"),
        "address": address,
        "lat": safe_float(place.get("y")),
        "lng": safe_float(place.get("x")),
        "phone": place.get("phone"),
        "place_url": place.get("place_url"),
...
def save_area_results(area, results, skipped_results):
    result_path = RESULT_DIR / f"cafe_{area['area_key']}_results.json"
    skipped_path = RESULT_DIR / f"cafe_{area['area_key']}_skipped_results.json"

    save_json(result_path, results)
    save_json(skipped_path, skipped_results)

    print(f"\n{area['area_name']} 완료")
    print(f"  저장 결과: {result_path}")
    print(f"  제외 결과: {skipped_path}")
    print(f"  최종 저장 장소 수: {len(results)}")
    print(f"  제외 장소 수: {len(skipped_results)}")
...
def collect_area(area):
    places, skipped_places = collect_kakao_places_for_area(area)
    ...
    save_area_results(area, results, all_skipped_results)

    return results, all_skipped_results
...
def main():
    ...
    for area in TARGET_AREAS:
        results, skipped_results = collect_area(area)
        all_results.extend(results)
        all_skipped_results.extend(skipped_results)
    ...
    print("전체 수집 완료")
    print("==============================")
    print(f"전체 저장 장소 수: {len(all_results)}")
    print(f"전체 제외 장소 수: {len(all_skipped_results)}")
    print(f"전체 결과: {RESULT_DIR / 'cafe_all_results.json'}")
    print(f"전체 제외 결과: {RESULT_DIR / 'cafe_all_skipped_results.json'}")
