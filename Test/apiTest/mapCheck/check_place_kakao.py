...
DATASET_CONFIGS = {
    "beach": {
        "input_path": Path("ExData/Cleaned/beach_places.json"),
        "search_suffix": "해수욕장",
        "category_keywords": ["해수욕장", "해변"],
    },
    "tourism": {
        "input_path": Path("ExData/Cleaned/tourism_places.json"),
        "search_suffix": "",
        "category_keywords": ["관광", "명소", "문화유적", "여행"],
    },
    "citypark": {
        "input_path": Path("ExData/Cleaned/citypark_places.json"),
        "search_suffix": "공원",
        "category_keywords": ["공원"],
    },
    "parking": {
        "input_path": Path("ExData/Cleaned/parking_places.json"),
        "search_suffix": "주차장",
        "category_keywords": ["주차장"],
    },
    "toilet": {
        "input_path": Path("ExData/Cleaned/toilet_places.json"),
        "search_suffix": "화장실",
        "category_keywords": ["화장실"],
    },
    "shelter": {
        "input_path": Path("ExData/Cleaned/shelter_places.json"),
        "search_suffix": "쉼터",
        "category_keywords": ["쉼터", "무더위쉼터", "대피소"],
    },
    "freewifi": {
        "input_path": Path("ExData/Cleaned/freewifi_places.json"),
        "search_suffix": "와이파이",
        "category_keywords": ["와이파이", "WiFi", "wifi"],
    },
}
...
def category_score(kakao_doc, dataset_config):
    category_name = kakao_doc.get("category_name", "")
    category_group_name = kakao_doc.get("category_group_name", "")
    place_name = kakao_doc.get("place_name", "")

    full_text = f"{category_name} {category_group_name} {place_name}"

    for keyword in dataset_config["category_keywords"]:
        if keyword in full_text:
            return 100

    # 관광지 계열은 카카오 카테고리가 넓게 잡히는 경우가 많아서 완화
    if "관광" in full_text or "명소" in full_text or "여행" in full_text:
        return 70

    return 0


def score_candidate(source_info, kakao_doc, dataset_config):
    ...
    cate_score = category_score(kakao_doc, dataset_config)
...
def make_queries(source_info, dataset_config):
    ...
    if name and suffix and suffix not in name:
        queries.append(f"{name} {suffix}")

    if region_hint and name:
        queries.append(f"{region_hint} {name}")

    if region_hint and name and suffix and suffix not in name:
        queries.append(f"{region_hint} {name} {suffix}")
...
def classify_best_candidate(best):
    ...
    category_score_value = best["score"]["category_score"]
    distance_m = best["score"]["distance_m"]

    if total_score >= 82 and name_score >= 60 and category_score_value >= 70:
        return "matched"

    if (
        name_score >= 92
        and category_score_value >= 70
...
def check_one_place(api_key, source_info, args, cache, dataset_config):
    queries = make_queries(source_info, dataset_config)
    ...
    for query in queries:
        ...
        for doc in data.get("documents", []):
            ...
            score = score_candidate(source_info, doc, dataset_config)
            ...
            if kakao_id not in merged_candidates:
                ...
                old_score = merged_candidates[kakao_id]["score"]["total_score"]

                if score["total_score"] > old_score:
                    merged_candidates[kakao_id] = {
                        "query": query,
                        "kakao": minimal_kakao_doc(doc),
                        "score": score,
...
def main():
    parser = argparse.ArgumentParser(
        description="정제 장소 JSON을 카카오 Local API로 검색해 matched / unmatched / review 파일을 생성합니다."
    )
    ...
    parser.add_argument(
        "--input",
        default=None,
        help="입력 정제 JSON 경로. 생략하면 dataset 기본 경로 사용",
    )
    ...
    if args.radius > 20000:
        raise ValueError("카카오 키워드 검색 radius는 최대 20000입니다.")

    dataset_config = DATASET_CONFIGS[args.dataset]

    env_path = load_dotenv_file()
    api_key, api_key_name = get_kakao_api_key()

    if not api_key:
        raise RuntimeError(
            ".env에서 카카오 REST API 키를 찾지 못했습니다. "
            "KAKAO_REST_API_KEY 또는 KAKAO_API_KEY 이름을 확인해 주세요."
        )
    ...
    cache_path = output_dir / f"{args.dataset}_kakao_search_cache.json"
    ...
    print(f".env 파일: {env_path if env_path else '찾지 못함'}")
    print(f"API 키 변수명: {api_key_name}")
    ...
    summary = {
        ...
        "settings": {
            ...
            "sort": args.sort,
            "sleep": args.sleep,
            "search_suffix": dataset_config.get("search_suffix"),
            "category_keywords": dataset_config.get("category_keywords"),
        },
        "started_at": started_at.isoformat(timespec="seconds"),
        "finished_at": finished_at.isoformat(timespec="seconds"),
    }

    matched_path = output_dir / f"{args.dataset}_matched.json"
    unmatched_path = output_dir / f"{args.dataset}_unmatched.json"
    review_path = output_dir / f"{args.dataset}_review.json"
    summary_path = output_dir / f"{args.dataset}_map_check_summary.json"

    save_json(matched_path, matched)
    ...
    print(f"matched 파일:   {matched_path}")
    print(f"unmatched 파일: {unmatched_path}")
    print(f"review 파일:    {review_path}")
    print(f"summary 파일:   {summary_path}")
    print(f"cache 파일:     {cache_path}")
